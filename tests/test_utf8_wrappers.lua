...
local DIR = "tests\\_unicode_fixture"
...
assert(file_exists(DIR .. "\\setup_done.txt"), "setup_done.txt is missing")
...
test("io.popen Unicode command arg", function()
  local cmd = 'cmd /c if exist "' .. BASE .. '_renamed.txt" (echo 1) else (echo 0)'
...
test("os.execute Unicode command arg", function()
  ...
  os.remove(marker)
  local ret = os.execute('cmd /c type nul > "' .. marker .. '"')
...
test("package.loadlib Unicode C path", function()
  local dll = DIR .. "\\lib_" .. NAME .. "\\modffi.dll"
...
os.execute('cmd /c rmdir /s /q "' .. DIR .. '"')
